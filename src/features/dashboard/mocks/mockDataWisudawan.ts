...
// ─── Fasilitas ITB (U03, SQ001–SQ012) — skala 1–4 ─────────────────────────────
...
// Score per item per faculty (avg, skala 4)
...
// Helper: average of items across all faculties
...
export const FASILITAS_ITB_AVG = FASILITAS_ITEMS.map((item, i) => ({
  ...item, avg: avgAcross(i),
...
function groupAvg(keys: string[]): number {
  ...
  return parseFloat((indices.map(i=>avgAcross(i)).reduce((s,v)=>s+v,0)/indices.length).toFixed(2));
}
...
// Faculty-level fasilitas comparison
...
export const PRODI_ITB_AVG = PRODI_ITEMS.map((item, i) => ({
  ...item, avg: prodiItemAvg(i),
})).sort((a,b) => a.avg - b.avg);
...
export const PRODI_FACULTY_AVG = FACULTIES.map(f => ({
  ...
  p12_pctSame: Math.round((PRODI_BY_FACULTY[f][11] / 4) * 100), // approx %
})).sort((a,b) => a.avg - b.avg);
...
export const PRODI_BOTTOM3_PER_FAC: FacultyItemRank[] = FACULTIES.flatMap(f =>
  PRODI_BY_FACULTY[f].map((v,i) => ({ faculty:f, item: PRODI_ITEMS[i].label, avg:v }))
...
// Rekomendasi (U02) distribution
export interface RekoData { faculty: string; notRekom: number; kualitasDosen: number; fasilitasAkademik: number; suasanaAkademik: number; lapanganKerja: number; jejaringAlumni: number; other: number; }
...
export const KARAKTER_ITEMS = [
  'Kejujuran','Memelihara komitmen','Menjaga emosi','Kepedulian terhadap orang lain',
  'Bersikap objektif','Tidak mudah menyerah','Patuh pada aturan/hukum',
];
...
export const SOFTSKILL_ITB_AVG = SOFTSKILL_ITEMS.map((label,i) => ({
  label, avg: skillItemAvg(SOFTSKILL_BY_FACULTY, i),
})).sort((a,b) => a.avg - b.avg);

export const KARAKTER_ITB_AVG = KARAKTER_ITEMS.map((label,i) => ({
  label, avg: skillItemAvg(KARAKTER_BY_FACULTY, i),
})).sort((a,b) => a.avg - b.avg);
...
// ─── Permasalahan (U06 + U07) ─────────────────────────────────────────────────

// % yang pernah mengalami masalah (skor > 1 dari skala 4-poin "tidak pernah"=1)
...
// U07: rata-rata kepuasan layanan DARI MEREKA YANG MENGALAMI MASALAH (skala 1-5)
...
// Gabungan semua item fasilitas + prodi + softskill
...
// ─── Suara Mahasiswa (Komentar Tematik) ──────────────────────────────────────

export interface IssueComment { label: string; count: number; sentiment: 'positive'|'neutral'|'negative'; }

export const ISSUES_POSITIF: IssueComment[] = [
...
export const ISSUES_NEGATIF: IssueComment[] = [
...
export const ISSUES_SARAN: IssueComment[] = [
