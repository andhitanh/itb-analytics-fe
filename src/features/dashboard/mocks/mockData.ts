...
// Previous semester deltas (subtract from latest to get earlier values; improves over time)
...
// Overall average per question (latest semester, across all faculties)
export const AVG_PER_QUESTION: number[] = QUESTIONS_SHORT.map((_, qi) => {
  const vals = FACULTIES.map(f => LATEST_SCORES[f][qi]);
  return Math.round((vals.reduce((s, v) => s + v, 0) / vals.length) * 100) / 100;
});

// Temporal: overall average (all faculties, all questions) per semester
export const TEMPORAL_AVG = SEMESTERS.map(sem => {
  const allScores = FACULTIES.flatMap(f => getScoreForSemester(f, sem));
  return {
    semester: sem === '2023/24-2' ? '23/24-2' : sem.replace('20','').replace('21','1').replace('/22','').slice(0,6),
    sem,
    avg: avgOf(allScores),
...
// Recalculate semester labels nicely
TEMPORAL_AVG.forEach((d, i) => { d.semester = SEMESTERS_SHORT[i]; });

// Faculty avg (latest semester)
export const FACULTY_AVG = FACULTIES.map(f => ({
  ...
  prev: avgOf(FACULTIES.map(f2 => getScoreForSemester(f2, '2023/24-1')).map(s => avgOf(s))),
}));

// Individual Q comparisons per faculty (for detail views)
...
// ─── Temporal Grade (Rata-Rata Nilai / IP Mahasiswa) ─────────────────────────
// Nilai mahasiswa rata-rata (skala 4.0) per semester — ITB keseluruhan & per-fakultas
// Catatan: menunjukkan tren pemulihan pasca-pandemi + perbaikan bertahap
...
// Sorted by avgScore desc (for charts)
export const GRADE_DIST_SORTED = [...GRADE_DIST].sort((a, b) => b.avgScore - a.avgScore);

// Per-faculty avg grade (skala 4.0) latest semester — diurutkan untuk bar chart
export const FACULTY_GRADE_LATEST = [...GRADE_DIST].sort((a, b) => b.avgScore - a.avgScore).map(d => ({
  faculty: d.faculty,
  avg: d.avgScore,
}));

// Overall ITB grade distribution
...
export interface GradingComp {
  faculty: string;
  UTS: number; UAS: number; Tugas: number; Kuis: number; Praktikum: number; Other: number;
}
