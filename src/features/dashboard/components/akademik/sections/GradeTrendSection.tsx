...
const GRADE_LINES = [
  { key:'itb',   name:'ITB',   color: chartColors.primary, width: 2.5, dash: undefined },
  { key:'sbm',   name:'SBM',   color: chartColors.success, width: 1.5, dash: '5 3'    },
  { key:'fsrd',  name:'FSRD',  color: chartColors.warning, width: 1.5, dash: '5 3'    },
  { key:'stei',  name:'STEI',  color: chartColors.mid,     width: 1.5, dash: '4 4'    },
  { key:'fmipa', name:'FMIPA', color: chartColors.light,   width: 1.5, dash: '4 4'    },
  { key:'fttm',  name:'FTTM',  color: chartColors.danger,  width: 1.5, dash: '3 3'    },
];
    ...
    <div className="grid grid-cols-2 gap-4">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Perkembangan IP dari semester ke semester</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex items-baseline gap-2.5 mb-1">
            ...
            <span className={`text-[12px] font-semibold px-2 py-0.5 rounded-md ${
              delta >= 0 ? 'bg-score-high-bg text-score-high-text' : 'bg-score-low-bg text-score-low-text'
            }`}>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={TEMPORAL_GRADE} margin={{ top: 4, right: 12, bottom: 0, left: -14 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#F0F2F7" />
              <XAxis dataKey="semester" tick={AXIS_STYLE} />
              <YAxis domain={[2.7, 3.6]} tick={AXIS_STYLE} tickFormatter={v => v.toFixed(1)} />
              <Tooltip content={<RechartsTooltip />} />
              <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 11 }} />
              <ReferenceLine y={3.0} stroke="#E74C3C" strokeDasharray="6 3" strokeWidth={1}
                label={{ value:'Min. 3.0', position:'insideTopRight', fontSize:10, fill:'#E74C3C' }} />
              {GRADE_LINES.map(l => (
                <Line key={l.key} dataKey={l.key} name={l.name} stroke={l.color}
                  strokeWidth={l.width} strokeDasharray={l.dash}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Diurutkan tertinggi · merah = di bawah 3.0</CardDescription>
        </CardHeader>
        <CardContent>
          <HBarChart
            data={gradeGroupData}
